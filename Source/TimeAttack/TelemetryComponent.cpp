...
void UTelemetryComponent::DrawTelemetry(UCanvas* Canvas, float& YL, float& YPos)
{
	...
	{
		float Value = 0.f;
		FString TelemetryComponent, TelemetryUnit;
		if (!Telemetry.Split("-", &TelemetryComponent, &TelemetryUnit))
		{
			TelemetryComponent = Telemetry;
			if (TelemetryComponent.Contains("Speed") ||
				TelemetryComponent.Contains("Velocity") ||
				TelemetryComponent.Contains("LinearVelocity"))
			{
				Telemetry.Append("-m/s");
			}
			else
			{
				Telemetry.Append("-deg/s");
			}
		}

		UObject* NewObject = nullptr;
		ATimeAttackPawn* MyPawn = Cast<ATimeAttackPawn>(GetWorld()->GetFirstPlayerController()->GetPawn());
		UFloatProperty* MyProperty = Cast<UFloatProperty>(UTimeAttackFunctionLibrary::RetrieveProperty(MyPawn, TelemetryComponent, NewObject));
		if (MyProperty != nullptr)
		{
			Value = MyProperty->GetPropertyValue_InContainer(NewObject);
		}
		else
		{
			TProperty<FVector, UProperty>* MyStructProperty = Cast<TProperty<FVector, UProperty>>(UTimeAttackFunctionLibrary::RetrieveProperty(MyPawn, TelemetryComponent, NewObject));
			FVector MyVector = MyStructProperty->GetPropertyValue_InContainer(NewObject);
			Value = MyVector.Size();
		}

		if (TelemetryUnit.Equals("km/h") && (TelemetryComponent.Contains("Speed") ||
			                                 TelemetryComponent.Contains("Velocity") ||
			                                 TelemetryComponent.Contains("LinearVelocity")))
		{
			Value = Value * 3.6f;
		...
		if (TelemetryUnit.Equals("rad/s") && (TelemetryComponent.Contains("AngularVelocity") ||
			                                  TelemetryComponent.Contains("SteerAngle") ||
			                                  TelemetryComponent.Contains("RotationAngle") ||
			                                  TelemetryComponent.Contains("SuspensionOffset")))
		{
			Value = Value * 0.017f;
		}
